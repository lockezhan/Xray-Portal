...
# 使用说明: 在部署时，请将相关占位符（如 <SUB_TOKEN>, <US_SERVER_IP> 等）替换为实际安全参数。
# =============================================================================
set -euo pipefail

SUB_TOKEN="<SUB_TOKEN>"
US_IP="<US_SERVER_IP>"
SUBMIRROR_PUB="ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAA...替代为您的submirror_key.pub真实公钥内容... submirror@clash-sub-us-to-nl"
...
SUB_TOKEN=${SUB_TOKEN}
US_IP=${US_IP}
US_DOMAIN=test.finalfinal.dpdns.org
NL_DOMAIN=gpt.finalfinal.dpdns.org
...
echo "=== 2. 写入 subpush_key ==="
cat > /opt/clash-sub-mirror/subpush_key << 'KEYEOF'
# 请在此处写入您在美国服务器生成的 /opt/clash-sub/scripts/subpush_key 的真实私钥内容 (包含头部和尾部声明)
KEYEOF
chmod 600 /opt/clash-sub-mirror/subpush_key
chown -R root:root /opt/clash-sub-mirror

echo "=== 3. 部署 submirror ssh 公钥 ==="
