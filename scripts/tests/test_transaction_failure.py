...
class TestTransactionFailure(unittest.TestCase):
    def setUp(self):
        ...
        # 复制必要的合并脚本与构建脚本至沙箱
        self.src_merge = os.path.join(self.base_dir, 'deploy/clash-sub/primary/extract_merge.py')
        self.src_rebuild = os.path.join(self.base_dir, 'deploy/clash-sub/primary/rebuild-clash-subscription.sh')
        self.src_validator = os.path.join(self.base_dir, 'deploy/clash-sub/primary/upload_validator.py')
        ...
        # 写入 config.env 基础配置
        self.primary_source_file = os.path.join(self.sandbox_dir, 'primary_source.yaml')
        self.config_env_file = os.path.join(self.scripts_dir, 'config.env')
        
        with open(self.config_env_file, 'w', encoding='utf-8') as f:
            f.write("PRIMARY_SOURCE={}\n".format(self.primary_source_file))
            f.write("INCOMING_DIR={}\n".format(self.incoming_dir))
            f.write("SECONDARY_SOURCE={}\n".format(os.path.join(self.sources_dir, 'secondary-full.yaml')))
            f.write("GENERATED={}\n".format(os.path.join(self.generated_dir, 'clash.yaml')))
            f.write("PUBLISHED_DIR={}\n".format(self.published_dir))
            f.write("BACKUP_DIR={}\n".format(self.backups_dir))
            # Secondary同步参数占位，忽略 SSH rsync
            f.write("SECONDARY_MIRROR_USER=dummy\n")
            f.write("SECONDARY_IP=127.0.0.1\n")
            f.write("SUB_TOKEN=test-token\n")
            f.write("LOG_FILE={}\n".format(os.path.join(self.logs_dir, 'rebuild.log')))

        # 写入有效的初始Primary源和历史快照，还有已发布的订阅
        self.valid_primary_data = "proxies:\n  - name: Primary-1\n    type: ss\n    server: primary1.example.com\n    port: 80\n"
        with open(self.primary_source_file, 'w') as f:
            f.write(self.valid_primary_data)
            
        self.valid_secondary_snapshot = "proxies:\n  - name: SECONDARY-OLD\n    type: ss\n    server: secondary.example.com\n    port: 80\n"
        with open(os.path.join(self.sources_dir, 'secondary-full.yaml'), 'w') as f:
            f.write(self.valid_secondary_snapshot)
    ...
    def test_invalid_ready_does_not_overwrite_snapshot(self):
        ...
        ready_path = os.path.join(self.incoming_dir, 'secondary-full.yaml.ready')
        ...
        # 验证旧 Secondary 快照未被覆盖
        with open(os.path.join(self.sources_dir, 'secondary-full.yaml'), 'r') as f:
            content = f.read()
        self.assertEqual(content, self.valid_secondary_snapshot)
    ...
    def test_external_validator_failure_restores_original_hashes(self):
        ...
        init_secondary_snapshot_hash = get_file_hash(os.path.join(self.sources_dir, 'secondary-full.yaml'))
        init_published_hash = get_file_hash(self.published_clash)

        # 模拟有新就绪的Secondary ready 配置 (这个配置是合法的，能通过 Python 合并)
        ready_path = os.path.join(self.incoming_dir, 'secondary-full.yaml.ready')
        valid_new_secondary = "proxies:\n  - name: SECONDARY-NEW\n    type: ss\n    server: new.example.com\n    port: 443\n"
        with open(ready_path, 'w') as f:
            f.write(valid_new_secondary)
        ...
        # 验证回滚后 Secondary 快照和 published/clash.yaml 哈希保持不变
        post_secondary_snapshot_hash = get_file_hash(os.path.join(self.sources_dir, 'secondary-full.yaml'))
        post_published_hash = get_file_hash(self.published_clash)

        self.assertEqual(post_secondary_snapshot_hash, init_secondary_snapshot_hash, "Secondary快照哈希被修改，回滚失败！")
    ...
    def test_upload_interrupted_leaves_ready_undamaged(self):
        ...
        env['SSH_ORIGINAL_COMMAND'] = 'upload-secondary'
        ...
        wrapper_path = os.path.join(self.base_dir, 'deploy/clash-sub/primary/subpush-cmd-wrapper')
