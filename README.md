...
- 一键导出 Clash Verge 配置（`/root/clash-verge.yaml`），并可选将配置发送到指定邮箱
...
4. 如需生成 Clash Verge 配置：
   ...
   - 输出文件位于：
     ...
     ```
   - 脚本生成配置后会询问是否将文件发送到指定邮箱。若选择发送，需依次输入：
     - 收件人邮箱
     - SMTP 服务器（如 `smtp.gmail.com`）
     - SMTP 端口（587 用于 STARTTLS，465 用于 SSL）
     - 发件人邮箱
     - SMTP 用户名（通常与发件人相同）
     - SMTP 密码（Gmail 建议使用[应用专用密码](https://myaccount.google.com/apppasswords)）
