...
- **新** - Clash 订阅 URL：`http://your.domain:8080/clash.yaml`，可在 Clash Verge 中直接添加订阅
...
======================================================
 Clash 订阅 URL（复制到 Clash Verge → 订阅）:
   http://vpn.example.com:8080/clash.yaml
======================================================
```
...
**⚡ 解决订阅链接国内被墙（重要！）⚡**
由于国内经常屏蔽 VPS 真实 IP 的 HTTP 端口导致订阅链接打不开，你可以利用 Cloudflare CDN 加速订阅服务（这是 `8080` 端口的优势）：
1. 在 Cloudflare 里新建一个**专门用于订阅的子域名**（例如 `sub.example.com`），指向你的 VPS IP。
2. 将这个 A 记录设为 **橙云（Proxied）**。
3. 你的普通节点域名（例如 `vpn.example.com`）保持 **灰云（仅 DNS）**。
4. 现在你可以通过 `http://sub.example.com:8080/clash.yaml` 来访问订阅链接！因为 8080 是 CF 支持代理的 HTTP 端口，订阅请求将通过 CF 的国内友好 CDN 中转，极大提升连接成功率！

其他选项：
```bash
sudo ./serve_clash.sh --port 8080       # 自定义端口（若要用CF橙云，请确认为 CF 支持的端口）
sudo ./serve_clash.sh --foreground      # 前台调试模式
sudo ./serve_clash.sh --uninstall       # 卸载服务
```

> 请确保云服务商安全组也放行了订阅端口（默认 TCP 8080）
...
放行 TCP/UDP 的 `20001–20003`，以及 TCP `8080`（订阅服务）。
...
- 订阅服务（8080）仅用于下发配置，不承载代理流量，结合 Cloudflare 橙云（Proxied）可有效防墙
- 保持系统与 Xray-core 最新
